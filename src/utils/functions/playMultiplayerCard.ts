...
const playMultiplayerCard = (
  ...
  if (card.number === 1) {
    ...
    // Note: We pass 'playersWithoutCard' implicitly via 'player' argument? No, handleCard8 takes 'player'.
    // But handleCard8 needs to check if player has another 8.
    ...
    // Normal card
    infoText = `${player.name} played ${card.number}`; // Or just "played card"
